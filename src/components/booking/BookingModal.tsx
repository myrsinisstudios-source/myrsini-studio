...
function DateBox({ label, value, onChange, placeholder, lang }: {
  ...
  const monthYear = formatMonthYear(value, lang)

  return (
    <label className="relative block bg-white border border-gray-200 p-4 text-center cursor-pointer hover:border-[#C9A96E] transition-colors group">
  ...
  return (
    <>
      ...
      >
        ...
        <span className="text-[10px] text-white/70">{m.from} €115</span>
      </button>
      ...
      <AnimatePresence>
        {open && (
          <>
            ...
            >
              <div className="pointer-events-auto relative w-full max-w-sm shadow-2xl"
                style={{ background: '#F8F5EF' }}>

                {/* Close */}
                <button
                  onClick={() => setOpen(false)}
                  className="absolute top-3 right-4 text-2xl leading-none text-gray-400 hover:text-gray-700 transition-colors z-10"
                  aria-label="Close"
                >
                  ×
                </button>
                ...
                <div className="px-8 pt-8 pb-6 text-center" style={{ background: '#EDE8DF' }}>
                  <div
                    className="inline-block text-white text-[10px] tracking-widest uppercase px-4 py-1.5 mb-4"
                ...
                <div className="px-6 py-6">
                      ...
                      return (
                        ...
                        >
                          ...
                          <div className="text-right">
                            ...
                            {!isStrike && (
                              <span className={`text-xs ml-0.5 ${isGold ? 'text-white/70' : 'text-[#C9A96E]/70'}`}>
                                {m.perNight}
                              </span>
                            )}
                          </div>
                  ...
                  {/* CTA */}
                  <button
                    className="w-full py-4 text-white text-xs font-bold tracking-widest uppercase hover:opacity-90 active:opacity-80 transition-opacity"
                    style={{ background: '#C9A96E' }}
                  >
                    {m.check}
                  </button>
